...
export interface ChecklistItem {
    ...
    async getChecklist(periodId: string, organizationId: string): Promise<ChecklistItem[]> {
        this.logger.log(`Generando checklist de cierre para el período ${periodId} en la organización ${organizationId}`);

        const period = await this.dataSource.getRepository(AccountingPeriod).findOneBy({ id: periodId, organizationId });
        if (!period) {
            throw new NotFoundException(`Período contable con ID "${periodId}" no encontrado.`);
        }

        const checklist: ChecklistItem[] = [];


        const unpostedEntriesCount = await this.dataSource.getRepository(JournalEntry).count({
            where: {
                organizationId,
        ...
        checklist.push({
            ...
            description: 'Verificar y procesar todos los asientos contables en borrador o pendientes de aprobación.',
        ...
        const unapprovedBillsCount = await this.dataSource.getRepository(VendorBill).count({
            where: {
                organizationId,
        ...
        checklist.push({
            ...
            description: 'Verificar y procesar todas las facturas de proveedor en borrador o pendientes de aprobación.',
        ...
        const unreconciledTxCount = await this.dataSource.getRepository(BankTransaction).count({
            where: {
        ...
        checklist.push({
            ...
            description: 'Ejecutar el proceso de revaluación de moneda extranjera para el cierre del período.',
            isCompleted: false,
            details: { message: "Este es un paso manual. Asegúrese de haber corrido el proceso antes de cerrar." },
        ...
        checklist.push({
            ...
            description: 'Ejecutar el proceso de depreciación de activos fijos para el mes.',
            isCompleted: false,
            details: { message: "Este es un paso manual. Asegúrese de haber corrido el proceso antes de cerrar." },
        ...
        const pendingApprovalsCount = await this.dataSource.getRepository(ApprovalRequest).count({
            where: {
                organizationId,
         ...
         checklist.push({
            ...
            description: 'Revisar y procesar todas las solicitudes de aprobación pendientes en el sistema.',
