...
export class DgiiEInvoiceProvider implements EInvoiceProvider {
  ...
  async sendInvoice(invoice: Invoice): Promise<EInvoiceResponse> {
    ...
    try {
      ...
      const { success, trackId, message } = this.parseDgiiResponse(soapResponse.data);
  ...
  private parseDgiiResponse(soapData: string): { success: boolean, trackId?: string, message?: string } {
